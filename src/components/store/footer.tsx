...
export function Footer({ store, categories, footer, menus = {}, pages = [], locale = "en" }: FooterProps) {
  const t = tFor(locale);
  ...
  const helpLinks: [string, string | null, string][] = [
    [t("nav.track"), null, "/track"],
    [t("nav.account"), null, "/account"],
    ...(pages.length ? pages.map((p): [string, string | null, string] => [p.title_en, p.title_bn, `/pages/${p.slug}`]) : ([["Terms & conditions", "শর্তাবলী", "/pages/terms"], ["Privacy policy", "গোপনীয়তা নীতি", "/pages/privacy"], ["Return & refund policy", "রিটার্ন ও রিফান্ড নীতি", "/pages/refund"]] as [string, string | null, string][])),
  ];
  const fallbackHelp: NavItem[] = helpLinks.map(([label_en, label_bn, href], i) => ({ id: `h${i}`, label_en, label_bn, href, icon: null, badge_label: null, badge_color: null, opens_new_tab: false, is_mega: false, mega: null, children: [] }));
  ...
  const copyright = locale === "bn" ? `© ${year} ${store.name}। ${t("misc.copyright")}` : f.copyright_en.replace("{year}", String(year)).replace("{store}", store.name);
    ...
    <footer className="bg-ink text-paper/80 border-ink-line mt-12 border-t pb-14 lg:pb-0">
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:grid-cols-2 lg:grid-cols-5">
        <div className="lg:col-span-1">
          ...
          {licence && <p className="mt-3 text-xs">{t("misc.trade_licence")}: {licence}</p>}
          {tin && <p className="text-xs">{t("misc.tin")}: {tin}</p>}
        </div>
        {columns.map((col) => (
          <div key={col.heading}>
        ...
        {f.show_contact_block && (
          <div>
            <h2 className="text-paper mb-3 text-sm font-semibold">{t("footer.contact")}</h2>
            <ul className="space-y-1.5 text-sm">
              {store.phone && <li><a href={`tel:${store.phone}`} className="hover:text-amber">{store.phone}</a></li>}
              {store.whatsapp && <li><a href={`https://wa.me/${store.whatsapp.replace(/\D/g, "")}`} className="hover:text-amber" rel="noopener">{t("footer.whatsapp")}</a></li>}
