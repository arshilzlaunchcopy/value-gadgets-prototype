...
export function CartLines({ lines, compact = false, onNavigate }: { lines: CartLine[]; compact?: boolean; onNavigate?: () => void }) {
  ...
  if (lines.length === 0) return <p className="text-muted-foreground py-8 text-center text-sm">Your cart is empty.</p>;
        ...
        <li key={l.id} className="flex gap-3 py-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex items-center justify-between gap-2">
              <div className="inline-flex items-center rounded-lg border" role="group" aria-label={`Quantity for ${l.title}`}>
                ...
                <span className="min-w-8 text-center text-sm tabular-nums">{l.quantity}</span>
              ...
              <span className="price text-sm">{formatBDT(l.line_total_bdt)}</span>
            </div>
            {l.available_qty < l.quantity && <p className="text-danger mt-1 text-xs">Only {l.available_qty} available now</p>}
          </div>
          <button type="button" onClick={() => change(l.id, 0)} disabled={pending} className="text-muted-foreground hover:text-danger self-start p-1" aria-label={`Remove ${l.title}`}>
