  ...
  return (
    <div className={`bg-paper text-ink mx-auto p-6 print:p-0 ${thermal ? "w-[4in]" : "w-[210mm]"}`}>
      <div className="mb-4 flex items-center gap-2 print:hidden">
        <PrintButton />
        <a href={`/admin/print/order/${o.id}${thermal ? "" : "?size=thermal"}`} className="rounded-lg border px-3 py-1.5 text-sm">{thermal ? "A4 layout" : "Thermal 4×6"}</a>
      ...
      {!thermal && (
        <section className="mt-6 break-before-page rounded-2xl border p-4 text-sm">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-lg font-bold">Invoice / চালান</p>
              <p className="text-xs">{store.name} · {store.address}</p>
              {store.trade_license && <p className="text-xs">Trade licence: {store.trade_license}</p>}
            </div>
            <div className="text-right text-xs">
              <p className="font-mono text-base font-bold">{o.order_number}</p>
              <p>{formatDate(o.placed_at)}</p>
              <p>{o.payment_method === "cod" ? "Cash on delivery" : "Paid online"}</p>
            </div>
          </div>
          <table className="mt-4 w-full text-xs">
            <thead className="border-b text-left">
              <tr><th className="py-1">Item</th><th className="py-1 text-right">Qty</th><th className="py-1 text-right">Unit</th><th className="py-1 text-right">Total</th></tr>
            </thead>
            <tbody>
              {(o.order_items ?? []).map((it, i) => (
            ...
            <tfoot>
              <tr><td colSpan={3} className="pt-2 text-right">Subtotal</td><td className="pt-2 text-right">{formatBDT(o.subtotal_bdt)}</td></tr>
              {o.discount_bdt > 0 && <tr><td colSpan={3} className="text-right">Discount</td><td className="text-right">-{formatBDT(o.discount_bdt)}</td></tr>}
              <tr><td colSpan={3} className="text-right">Delivery</td><td className="text-right">{formatBDT(o.shipping_bdt)}</td></tr>
              <tr className="font-bold"><td colSpan={3} className="text-right">Total</td><td className="text-right">{formatBDT(o.total_bdt)}</td></tr>
            </tfoot>
          </table>
        </section>
      )}
